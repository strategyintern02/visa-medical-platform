...
app.get('/api/centres', async (req, res) => {
  try {
    ...
    // Empanelment / de-panel / re-empanel writes are the CORE of this tool and must be
    // visible the moment anyone reloads — so this read is NEVER cached. Do NOT add s-maxage
    // here (edge-caching made writes take up to ~60s to appear on refresh). First-load speed
    // comes from lazy-loading xlsx + deferring the map libs, not from caching the data.
